...
public class AndroidPowerMeterService implements IPowerMeterService {
    private static final UUID CYCLING_POWER_SERVICE_UUID = uuid16(0x1818);
    private static final UUID CYCLING_POWER_MEASUREMENT_UUID = uuid16(0x2A63);
    ...
    private final CopyOnWriteArrayList<OnPowerData> listeners = new CopyOnWriteArrayList<>();
    ...
    public AndroidPowerMeterService(Context context) {
    ...
    public void selectNewDevice() {
        ...
        try {
            ...
            ScanFilter filter = new ScanFilter.Builder().setServiceUuid(new ParcelUuid(CYCLING_POWER_SERVICE_UUID)).build();
    ...
    @Override
    public void subscribeForPowerData(OnPowerData listener) {
        listeners.addIfAbsent(listener);
    }

    @Override
    public void unsubscribeForPowerData(OnPowerData listener) {
    ...
    private final BluetoothGattCallback gattCallback = new BluetoothGattCallback() {
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            BluetoothGattCharacteristic characteristic = service.getCharacteristic(CYCLING_POWER_MEASUREMENT_UUID);
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            ...
            PowerMeterData data = parsePowerData(value);
            for (OnPowerData listener : listeners) {
                listener.onPowerData(data);
            }
        }
    };

    private PowerMeterData parsePowerData(byte[] value) {
        int flags = uint16LE(value, 0);
        int instantaneousPower = int16LE(value, 2);
        int index = 4;

        PowerMeterData powerData = new PowerMeterData(System.currentTimeMillis(), instantaneousPower);
        if ((flags & 0x01) != 0) {
            powerData.balance = uint8(value, index) / 2.0;
            index += 1;
        }
        if ((flags & 0x04) != 0) {
            powerData.accumulatedTorque = uint16LE(value, index) / 32.0;
            index += 2;
        }
        if ((flags & 0x20) != 0) {
            powerData.cumulativeCrankRevolutions = (long) uint16LE(value, index);
            index += 2;
            powerData.lastCrankEventTimestamp = (long) uint16LE(value, index);
            index += 2;
        }
        return powerData;
    }
    ...
    private static int int16LE(byte[] value, int offset) {
        int unsigned = uint16LE(value, offset);
